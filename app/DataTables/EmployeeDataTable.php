    ...
    type="button" data-bs-toggle="dropdown" data-boundary="window" aria-haspopup="true" aria-expanded="false" data-bs-reference="parent">
        <i class="fa-solid fa-ellipsis-vertical fa-xl"></i>
            ...
            <div class="dropdown-menu dropdown-menu-end py-2">';
                $html .= '<a href="#" data-bs-toggle="modal" data-bs-target="#editModal' . $model->id . '" data-id="' . $model->id . '" class="dropdown-item editEmployee">Edit</a>';
                $html .= '<a href="#" data-bs-toggle="modal" data-bs-target="#deleteModal' . $model->id . '" data-id="' . $model->id . '" class="dropdown-item deleteEmployee">Delete</a>';
            ...
            ->editColumn('manager_id', function ($model) {
                return $model->manager ? $model->manager->name : 'No Manager';
            })
            // ->editColumn('image', function ($model) {
            //     return $model->image ? $model->image : 'No Image';
            // })
            ->addColumn('department', function ($model){
                return $model->department ? $model->department->name : 'N/A';
            })



            ->rawColumns(['full_name','first_name','last_name','salary', 'action'])
            ->setRowId('id');
    }
...
window.LaravelDataTables['employees_table'].columns.adjust().draw();
window.LaravelDataTables['employees_table'].on('select deselect',function(){
var count = window.LaravelDataTables['employees_table'].rows({ selected: true }).count();
$('.selected-item').text(count);
$('.selected-badge').text(count);
window.LaravelDataTables['employees_table'].button('copy:name').enable(count > 0);
window.LaravelDataTables['employees_table'].button(2).enable(count > 0);
window.LaravelDataTables['employees_table'].button('export:name').enable(count > 0);


// Clear the set before updating
checkBoxIds.clear();
// Iterate over each selected row
window.LaravelDataTables['employees_table'].rows({ selected: true }).every(function(rowIdx) {
...
window.LaravelDataTables['employees_table'].rows({ selected: false }).every(function(rowIdx) {
    // Get the corresponding checkbox and check it
    var checkbox = $(this.node()).find('td:eq(0) input[type=\"checkbox\"]');
...
localStorage.setItem('employees_checkBoxIdsArray', JSON.stringify(checkBoxIdsArray))
            ...
            arrOfFilterBtn.push({
                ...
                action: function (e, dt, node, config, cb) {
                    ...
                    $('#' + id).prop('checked', function (_, oldProp) {
                        if (oldProp) {
window.LaravelDataTables['employees_table'].columns(index + 1).search("").draw();
    ...
    arrOfFilterBtn.push({
        ...
        action: function(e, dt, node, config, cb) {
            ...
            $('.filter-modal input[type="text"]').on('input', function() {
                ...
                window.LaravelDataTables['employees_table'].columns(filterIndex).search(searchValue).draw();
            });
        }
    });
...
JS;
      ...
      $('.selected-item').text(window.LaravelDataTables['employees_table'].rows({ selected: true }).count());
      $('.selected-badge').text(window.LaravelDataTables['employees_table'].rows({ selected: true }).count());
    },lengthMenu: [
        [100,200],
        [100,200]
    ]",
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('full_name')->title('Full Name')->addClass('text-center'),
            Column::make('first_name')->addClass('text-center'),
            Column::make('last_name')->addClass('text-center'),
            Column::make('manager_id')->title('Manager')->addClass('text-center'),
            Column::make('salary')->addClass('text-center'),
            Column::make('department')->addClass('text-center'),
            // Column::make('image')->title('Image')->addClass('text-center'),
